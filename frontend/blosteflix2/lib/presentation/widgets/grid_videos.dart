...
class _GridVideosState extends State<GridVideos> {
  ...
  void didUpdateWidget(covariant GridVideos oldWidget) {
    ...
    if (widget.categoriaSeleccionada?.id != oldWidget.categoriaSeleccionada?.id) {
      setState(() {
        _videos.clear();
        _currentPage = 0;
        _hasMore = true;
  ...
  Future<void> _loadMoreVideos() async {
    ...
    try {
      ...
      final response = await CatalogoLocator()
          .getVideosUseCase
          .execute(
            page: _currentPage,
            size: _sizeToLoad,
    ...
    return SliverFillRemaining(
      ...
      child: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.video_collection_outlined, size: 80, color: Colors.grey),
            const SizedBox(height: 16),
            Text(
